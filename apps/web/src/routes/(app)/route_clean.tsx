...
import { createEffect, Show, For, Accessor, createSignal } from "solid-js";
...
export const Route = createFileRoute("/(app)/route_clean")({
...
function RouteComponent() {
  ...
  const conversationsState = useSequentialConversations(data.data?.email ?? "");

  // WebSocket connection setup - simplified and working
  const [wsConnected, setWsConnected] = createSignal(false);

  createEffect(() => {
    const accountId = data.data?.accountId;

    if (accountId && !websocket) {
      console.log("Creating WebSocket connection for account:", accountId);

      const wsUrl = "wss://47k9blmjq8.execute-api.us-east-1.amazonaws.com/dev";
      ...
      websocket.onclose = (event) => {
        ...
        // Reset websocket to allow reconnection
        websocket = undefined;
      };
      ...
      websocket.onmessage = (event) => {
        try {
          ...
          if (wsData.type === "new_message") {
            ...
            const currentConvoQuery = conversationsState.query as Accessor<
              UseQueryResult<Conversation[], Error>
            >;
            const currentData = currentConvoQuery();
            ...
            if (convo && data.data?.email) {
              ...
              // If conversations aren't loaded yet, trigger a refetch
              if (!currentData.data && !currentData.isLoading) {
                console.log("Conversations not loaded, triggering refetch...");
                currentConvoQuery().refetch?.();
              }
            }
          }
        } catch (err) {
